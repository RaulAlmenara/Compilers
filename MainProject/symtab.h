...
/* The symbol table struct is an union
(a new data type with more than one member in the same memory location)
 with all the types of the different variables   */

typedef enum{INT_TYPE,FLOAT_TYPE,STRING_TYPE,BOOL_TYPE,ID_TYPE} type;
typedef enum{VAL_FALSE, VAL_TRUE} boolean;
typedef struct{
	int size;
	int *values;
}list_Int;
...
typedef sym_table sym_value_type;
